class Api::SuggestedusersController < ApplicationController
  ...
  respond_to :json
  def index
    @user_ids = current_user.friends.map { |f| f.own_posts.where(poster: current_user).group(:recipient_id).count }.sort_by {|k,v| v}.last(4).map { |h| h.keys[0] }
    if @user_ids.length < 4
      @friends = current_user.friends.last(4).map { |x| x.id }
      @numFriends = 4 - @user_ids.length
